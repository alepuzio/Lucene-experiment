...
/***
 * from https://www.baeldung.com/lucene
 **/
public class InMemoryLuceneIndex {
    ...
    public void indexDocument(String title, String body) {
        try {
            Document document = new Document();//document with no fields
            document.add(new TextField("title", title, Field.Store.YES));//stored content
    ...
    /**
     * a search query and search the index for the added document
     * */
    public List<Document> searchIndex(String inField, String queryString) {
        try {
            ...
            TopDocs topDocs = searcher.search(query, 10);//10 is the limit of the result
